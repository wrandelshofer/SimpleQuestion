...
// by hand).
// PATCH Werner Randelshofer: Added support for unicodeKind.
public class TextScanner extends Scanner {
     ...
     */
    private static int unicodeKind = 1;
    ...
    protected int read() {
        state = 0;
        int ch = buffer[start];
        int kind = (ch >= kinds.length) ? unicodeKind : kinds[ch];
        int type;
        while (true) {
            ...
            if (start >= end) {
                ...
                ch = buffer[start];
                kind = (ch >= kinds.length) ? unicodeKind : kinds[ch];
            }
        }
        state = WHITESPACE;
        return type;
